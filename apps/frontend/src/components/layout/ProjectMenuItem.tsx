import { useState } from 'react'
import { Link } from '@tanstack/react-router'
import { useDraggable } from '@dnd-kit/core'
import { CSS } from '@dnd-kit/utilities'
import { ChevronRight } from 'lucide-react'
import { getProjectIcon } from '@/components/configure/ProjectIconPicker'
import { cn } from '@/lib/utils'
import { useWorkflows } from '@/hooks/queries'
import {
  SidebarMenuItem,
  SidebarMenuButton,
  SidebarMenuSub,
} from '@/components/ui/sidebar'
import { WorkflowMenuItem } from '@/components/layout/WorkflowMenuItem'
import type { Project } from '@potato-cannon/shared'

interface ProjectMenuItemProps {
  project: Project
  isActive: boolean
  hasActiveSessions: boolean
  hasPendingQuestions: boolean
  currentWorkflowId?: string | null
}

export function ProjectMenuItem({
  project,
  isActive,
  hasActiveSessions,
  hasPendingQuestions,
  currentWorkflowId,
}: ProjectMenuItemProps) {
  const Icon = getProjectIcon(project.icon || 'package')
  const colorStyle = project.color ? { color: project.color } : undefined

  const { attributes, listeners, setNodeRef, isDragging, transform } = useDraggable({
    id: project.id,
    data: { projectId: project.id, folderId: project.folderId },
  })

  const { data: workflows } = useWorkflows(project.id)
  const [isExpanded, setIsExpanded] = useState(true)

  const hasMultipleWorkflows = workflows && workflows.length > 1
  const singleWorkflow = workflows && workflows.length === 1 ? workflows[0] : null

  // Single workflow: link directly to the workflow board
  if (singleWorkflow) {
    return (
      <SidebarMenuItem
        ref={setNodeRef}
        className={cn(
          hasActiveSessions && 'thinking-shimmer',
          isDragging && 'opacity-50'
        )}
        style={{ transform: CSS.Translate.toString(transform) }}
        {...listeners}
        {...attributes}
      >
        <SidebarMenuButton
          asChild
          isActive={isActive}
          tooltip={project.displayName || project.id}
        >
          <Link
            to="/projects/$projectId/workflows/$workflowId/board"
            params={{ projectId: project.slug, workflowId: singleWorkflow.id }}
          >
            <Icon className="h-4 w-4" style={colorStyle} />
            <span style={colorStyle} className="flex-1">
              {project.displayName || project.id}
            </span>
            {hasPendingQuestions && (
              <span className="shrink-0 flex items-center justify-center w-2 h-2 bg-accent rounded-full" />
            )}
          </Link>
        </SidebarMenuButton>
      </SidebarMenuItem>
    )
  }

  // Multiple workflows: expandable with workflow children
  if (hasMultipleWorkflows) {
    return (
      <>
        <SidebarMenuItem
          ref={setNodeRef}
          className={cn(
            hasActiveSessions && 'thinking-shimmer',
            isDragging && 'opacity-50'
          )}
          style={{ transform: CSS.Translate.toString(transform) }}
          {...listeners}
          {...attributes}
        >
          <SidebarMenuButton
            isActive={isActive}
            tooltip={project.displayName || project.id}
            onClick={() => setIsExpanded((prev) => !prev)}
          >
            <ChevronRight
              className={cn(
                'h-3 w-3 shrink-0 transition-transform',
                isExpanded && 'rotate-90'
              )}
            />
            <Icon className="h-4 w-4" style={colorStyle} />
            <span style={colorStyle} className="flex-1">
              {project.displayName || project.id}
            </span>
            {hasPendingQuestions && (
              <span className="shrink-0 flex items-center justify-center w-2 h-2 bg-accent rounded-full" />
            )}
          </SidebarMenuButton>
        </SidebarMenuItem>
        {isExpanded && (
          <SidebarMenuSub>
            {workflows.map((workflow) => (
              <WorkflowMenuItem
                key={workflow.id}
                workflow={workflow}
                projectSlug={project.slug}
                isActive={workflow.id === currentWorkflowId}
              />
            ))}
          </SidebarMenuSub>
        )}
      </>
    )
  }

  // No workflows yet (loading or empty): fall back to project board link
  return (
    <SidebarMenuItem
      ref={setNodeRef}
      className={cn(
        hasActiveSessions && 'thinking-shimmer',
        isDragging && 'opacity-50'
      )}
      style={{ transform: CSS.Translate.toString(transform) }}
      {...listeners}
      {...attributes}
    >
      <SidebarMenuButton
        asChild
        isActive={isActive}
        tooltip={project.displayName || project.id}
      >
        <Link to="/projects/$projectId/board" params={{ projectId: project.slug }}>
          <Icon className="h-4 w-4" style={colorStyle} />
          <span style={colorStyle} className="flex-1">
            {project.displayName || project.id}
          </span>
          {hasPendingQuestions && (
            <span className="shrink-0 flex items-center justify-center w-2 h-2 bg-accent rounded-full" />
          )}
        </Link>
      </SidebarMenuButton>
    </SidebarMenuItem>
  )
}
