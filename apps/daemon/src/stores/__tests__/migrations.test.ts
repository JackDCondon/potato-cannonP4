...
describe('V27 migration - chat queue cleanup', () => {
  it('drops chat_queue_items table', () => {
    ...
    const result = db
      ...
      .get();
    assert.equal(result, undefined, 'chat_queue_items table should be removed by V27 migration');
  });
  ...
  it('drops chat_delivery_events table', () => {
    ...
    const result = db
      ...
      .get();
    assert.equal(result, undefined, 'chat_delivery_events table should be removed by V27 migration');
  });

  it('bumps the schema version to 27', () => {
    const db = new Database(':memory:');
    runMigrations(db);
