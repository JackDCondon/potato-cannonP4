...
describe('V17 migration history - provider route indexes survive queue cleanup', () => {
  it('keeps provider channel route indexes after legacy queue tables are dropped', () => {
    const db = new Database(':memory:');
    runMigrations(db);

    const indexes = db.pragma('index_list(provider_channels)') as Array<{ name: string }>;
    const indexNames = new Set(indexes.map((index) => index.name));
    assert.ok(indexNames.has('idx_provider_channels_route_thread_id'));
    assert.ok(indexNames.has('idx_provider_channels_route_slack_thread'));
    ...
    const eventTable = db
      ...
      .get();
    assert.equal(queueTable, undefined);
...
describe('V13 backfill Ã¢â‚¬â€ runBackfillV13', () => {
  ...
  it('is idempotent Ã¢â‚¬â€ running twice does not create duplicate workflows', () => {
    const db = new Database(':memory:');
    runMigrations(db);
  ...
  it('does not overwrite existing workflow_id on tickets that already have one', () => {
    ...
    // Exactly one project_workflows row must exist Ã¢â‚¬â€ no duplicate is_default=1 rows
    const workflows = db
  ...
  it('uses pre-existing is_default=1 workflow (non-Default name) for backfill, does not create extra row', () => {
    ...
    // Exactly one project_workflows row must exist Ã¢â‚¬â€ no new 'Default' row should have been inserted
    const workflows = db
...
describe('V21 migration - brainstorm-ticket linkage', () => {
  ...
  it('is idempotent Ã¢â‚¬â€ running V21 migration twice does not error', () => {
    const db = new Database(':memory:');
    runMigrations(db);
...
describe('V23 migration - epic color and icon customization', () => {
  ...
  it('is idempotent Ã¢â‚¬â€ running V23 migration twice does not error', () => {
    const db = new Database(':memory:');
    runMigrations(db);
...
describe('V24 migration - token usage columns', () => {
  it('adds input_tokens and output_tokens columns to sessions', () => {
    const db = new Database(':memory:');
    runMigrations(db);

    const columns = db.pragma('table_info(sessions)') as Array<{ name: string }>;
    const names = new Set(columns.map((column) => column.name));
    assert.ok(names.has('input_tokens'), 'sessions table should have input_tokens column');
...
describe('V25 migration - ticket pause state columns', () => {
  it('adds paused, pause_reason, pause_retry_at, pause_retry_count columns to tickets', () => {
    const db = new Database(':memory:');
    runMigrations(db);

    const columns = db.pragma('table_info(tickets)') as Array<{ name: string }>;
    const names = new Set(columns.map((column) => column.name));
    assert.ok(names.has('paused'), 'tickets table should have paused column');
    assert.ok(names.has('pause_reason'), 'tickets table should have pause_reason column');
    assert.ok(names.has('pause_retry_at'), 'tickets table should have pause_retry_at column');
...
describe('V26 migration - per-project p4 connection settings', () => {
  it('adds p4_use_env_vars, p4_port, and p4_user columns to projects', () => {
    const db = new Database(':memory:');
    runMigrations(db);

    const columns = db.pragma('table_info(projects)') as Array<{ name: string }>;
    const names = new Set(columns.map((column) => column.name));
    assert.ok(names.has('p4_use_env_vars'), 'projects table should have p4_use_env_vars column');
    assert.ok(names.has('p4_port'), 'projects table should have p4_port column');
...
describe('V28 migration - chat queue cleanup', () => {
  it('drops chat_queue_items table', () => {
    const db = new Database(':memory:');
    runMigrations(db);
    ...
    const result = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='chat_queue_items'")
      .get();
    assert.equal(result, undefined, 'chat_queue_items table should be removed by V28 migration');
  });

  it('drops chat_delivery_events table', () => {
    const db = new Database(':memory:');
    runMigrations(db);
    ...
    const result = db
      .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='chat_delivery_events'")
      .get();
    assert.equal(result, undefined, 'chat_delivery_events table should be removed by V28 migration');
  });

  it('bumps the schema version to 28', () => {
    const db = new Database(':memory:');
    runMigrations(db);
...
describe('V22 migration - PM fields and board settings', () => {
  ...
  it('adds pm_config column to brainstorms', () => {
    const db = new Database(':memory:');
    runMigrations(db);

    const columns = db.pragma('table_info(brainstorms)') as Array<{ name: string }>;
    const names = new Set(columns.map((column) => column.name));
    assert.ok(names.has('pm_config'), 'brainstorms table should have pm_config column');
  });
  ...
  it('is idempotent Ã¢â‚¬â€ running V22 migration twice does not error', () => {
    const db = new Database(':memory:');
    runMigrations(db);
